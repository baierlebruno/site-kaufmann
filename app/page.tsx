...
function Logo({ className = '' }: { className?: string }) {
  ...
  if (failed) {
    return (
      <div className={`logo logo-fallback ${className}`}>
        <span>AUTO SOM</span>
        <strong>KAUFMANN</strong>
      </div>
    )
  }
  return (
    <img
      src="/images/logo-auto-som-kaufmann.png"
      alt="Auto Som Kaufmann"
...
function Fachada() {
  ...
  if (failed) {
    return (
      <div className="about-image about-fallback" role="img" aria-label="Auto Som Kaufmann">
        <span>AUTO SOM</span>
        <strong>KAUFMANN</strong>
      </div>
    )
  ...
  return (
    <img
      src="/images/fachada-auto-som-kaufmann.jpg"
      alt="Fachada da Auto Som Kaufmann em Toledo"
      className="about-image about-img"
      onError={() => setFailed(true)}
    />
  )
}
    ...
    <main>
      <header className="site-header">
        <div className="container nav-wrap">
          <a href="#inicio" onClick={close} aria-label="Auto Som Kaufmann - início">
            <Logo />
          </a>
          <nav id="main-navigation" className={open ? 'nav open' : 'nav'} aria-label="Navegação principal">
            {navItems.map(([label, id], i) => (
              <a key={id} href={`#${id}`} onClick={close} className={i === 0 ? 'active' : ''}>
                {label}
              </a>
            ))}
          </nav>
          <a className="button button-small nav-cta" href={whatsapp} target="_blank" rel="noopener noreferrer">
            <FaWhatsapp size={17} /> WhatsApp
          </a>
          <button type="button" className="menu-toggle" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="main-navigation" aria-label={open ? 'Fechar menu' : 'Abrir menu'}>
            {open ? <X /> : <Menu />}
          </button>
        </div>
      </header>
      ...
      <section id="inicio" className="hero">
        ...
        <div className="container hero-content">
          <h1>
            Tradição, qualidade e<br /> confiança há mais de <em>40 anos</em>
          </h1>
          <p className="hero-subtitle">
            Som automotivo, acessórios, alarmes e<br className="desktop" /> películas em Toledo e região.
          </p>
          <div className="hero-actions">
            <a className="button" href={whatsapp} target="_blank" rel="noopener noreferrer">
              <FaWhatsapp size={19} /> Solicitar orçamento no WhatsApp
            </a>
            <a className="button button-outline" href="#servicos">
              Conhecer serviços <ArrowRight size={17} />
            </a>
          </div>
          <div className="proofs">
            <div>
              <CalendarDays size={22} />
              <span>Desde<br /><b>1985</b></span>
            </div>
            <div>
              <Star size={22} />
              <span><b>4,9</b><br />no Google</span>
            </div>
            <div>
              <ShieldCheck size={22} />
              <span>Autorizado<br /><b>3M</b></span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container">
              ...
              return (
                <article className="service-card" key={service.title}>
                  ...
                  <div className="service-copy">
                    <div className="service-icon"><ServiceIcon size={18} /></div>
                    <h3>{service.title}</h3>
                    <p>{service.text}</p>
                  </div>
                </article>
          ...
          <div className="brands">
            ...
            <div className="brand-row">
              {brands.map((brand) => (
                <BrandLogo key={brand.name} name={brand.name} file={brand.file} />
              ))}
            </div>
          </div>
        </div>
      ...
      <section id="produtos" className="section products">
        <div className="container">
          ...
          <a className="product-note" href={whatsapp} target="_blank" rel="noopener noreferrer">
            <FaWhatsapp size={17} /> Consulte modelos e disponibilidade pelo WhatsApp
          </a>
        </div>
      </section>
      ...
      <section id="contato" className="section contact">
        <div className="container contact-grid">
          ...
          <div className="address">
            <MapPin size={20} />
            <p>Rua Adilson Bier, 66<br />Jardim Porto Alegre<br />Toledo - PR</p>
            <a className="button button-small" href={maps} target="_blank" rel="noopener noreferrer">
              Abrir no Google Maps <ExternalLink size={15} />
            </a>
          </div>
          <iframe
            className="map"
            src="https://www.google.com/maps?q=Rua+Adilson+Bier,+66,+Jardim+Porto+Alegre,+Toledo,+PR&output=embed"
            title="Localização da Auto Som Kaufmann no Google Maps"
            loading="lazy"
        ...
        <div className="container cta-inner">
          ...
          <div className="cta-grid">
            ...
            <div className="cta-action">
              <a className="button" href={whatsapp} target="_blank" rel="noopener noreferrer">
                <FaWhatsapp size={19} /> Falar no WhatsApp
              </a>
      ...
      <a className="floating-whatsapp" href={whatsapp} target="_blank" rel="noopener noreferrer" aria-label="Falar no WhatsApp">
        <FaWhatsapp size={26} />
      </a>
    </main>
  )
